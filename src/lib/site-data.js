...
(function () {
	...
	/* Resolve a path written inside a content file relative to that file, so
	   posts.yml can say `file: posts/intro.yml` and mean content/posts/intro.yml.
	   An absolute path or a full URL is left alone. */
	function relativeTo(base, ref) {
	...
	/* ---------- includes ----------
	   An entry may be written inline, or delegated to its own file:

	     - file: posts/rare-event-kinetics.yml

	   That keeps one heavy post (long body, tables, code, math) in its own
	   file while posts.yml stays a short, readable running order. Keys given
	   alongside `file:` win over the ones in the included file, so the index
	   can override a title or flip `draft: true` without opening it.

	   Includes are resolved eagerly and in parallel: the archive pages compute
	   "N min read" from every entry's body, so the bodies have to be present
	   before the list renders. One level deep only — an included file is an
	   entry, not another index. */
	function resolveIncludes(path, list) {
		var jobs = list.map(function (entry) {
			if (!entry || typeof entry !== "object" || !entry.file) {
				return Promise.resolve(entry);
			}
			var target = relativeTo(path, String(entry.file));
			return loadMap(target).catch(function (e) {
				// A missing or unreadable include must cost one entry, never the
				// whole page — otherwise a single typo blanks the archive.
				warn(path, 'include "' + entry.file + '" could not be read (' +
					(e && e.message || e) + ') — the entry is skipped.');
				return null;
			}).then(function (loaded) {
				if (!loaded || !Object.keys(loaded).length) {
					warn(path, 'include "' + entry.file + '" is empty — the entry is skipped.');
					return null;
				}
				var merged = {};
				Object.keys(loaded).forEach(function (k) { merged[k] = loaded[k]; });
				// Keys written next to `file:` override the included file.
				Object.keys(entry).forEach(function (k) {
					if (k !== "file") merged[k] = entry[k];
				});
	...
	function load(path) {
		if (!cache[path]) {
			cache[path] = fetch(path, { cache: "no-cache" })
				...
				.then(function (list) {
					return resolveIncludes(path, list);
				});
		}
		return cache[path];
	}
	...
	function ready() {
		if (!readyPromise) {
			readyPromise = loadMap("content/site.yml")
				.catch(function () {
					return {};
				})
				.then(function (cfg) {
	...
	/* ---------- tags ----------
	   tagsOf normalises the three shapes a tags: field can take
	   (list, single string, absent). The archive pages search across tags
	   via their own free-text box; there is no shared filter helper. */
